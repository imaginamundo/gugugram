...
/** Reproduces the oldest format: `<hash>.<salt>`, 10,000 iterations, 512-byte key. */
...
/**
 * Reproduces the interim `pbkdf2$` format currently on main: 210,000
 * iterations but still a 512-byte key. Every account created or reset since
 * that change carries one of these, so they must keep verifying.
 */
const interimHash = (password: string) => {
	const salt = crypto.randomBytes(128).toString("base64");
	const hash = crypto.pbkdf2Sync(password, salt, 210_000, 512, "sha512");
...
describe("validatePassword", () => {
	...
	it("still accepts passwords stored in the interim pbkdf2$ format", () => {
		expect(validatePassword(interimHash("correct horse"), "correct horse")).toBe(true);
		expect(validatePassword(interimHash("correct horse"), "wrong")).toBe(false);
	});
...
describe("needsRehash", () => {
	...
	it("flags interim hashes, which pay for 512 bytes of key to get 64 bytes of security", () => {
		expect(needsRehash(interimHash("correct horse"))).toBe(true);
	});
